...
public class QuestManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        foreach (Quest quest in questMap.Values)
        {
            ...
            if (quest.state == QuestState.REQUIREMENTS_NOT_MET && CheckRequirementsMet(quest))
            {
                ...
                DialogueManager.Instance.EnableQuestChoice(quest.info.id, QuestState.CAN_START);
            }
        }
    }
    ...
    private void AdvanceQuest(string id)
    {
        ...
        else
        {
            ...
            DialogueManager.Instance.EnableQuestChoice(quest.info.id, QuestState.CAN_FINISH);
        }
    }
    ...
    {
        // 모든 퀘스트 스크립터블 오브젝트를 불러온다
        QuestSO[] allQuest = Resources.LoadAll<QuestSO>("Quests");

        Dictionary<string, Quest> idToQuestMap = new Dictionary<string, Quest>();
        foreach (QuestSO quest in allQuest)
        {
            if (idToQuestMap.ContainsKey(quest.id))
            {
                Debug.LogWarning("중복된 퀘스트ID 발견: " + quest.id);
            }
             idToQuestMap.Add(quest.id, LoadQuest(quest));
        }
